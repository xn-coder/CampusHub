...
export default function AdminStudentScoresPage() {
  ...
  const handlePrintResult = (score: StudentScore) => {
    ...
    doc.setFontSize(12);
    doc.text(`School: ${schoolName}`, 14, 32);
    doc.text(`Student: ${studentName}`, 14, 40);
    doc.text(`Exam: ${examName}`, 14, 46);
    
    autoTable(doc, {
        startY: 55,
        head: [['Subject', 'Score', 'Max Marks']],
        body: [[subjectName, String(score.score), String(maxMarks)]],
        theme: 'striped',
    ...
    doc.setFontSize(10);
    doc.text("This is an official document generated by CampusHub.", 14, finalY + 15);
    
    doc.save(`Mark_Sheet_${studentName.replace(/\s/g, '_')}_${examName.split(' - ')[0].replace(/\s/g, '_')}.pdf`);

    toast({
        title: "Downloading Report",
        description: `A mark sheet for ${studentName} is being downloaded.`
    });
  };
