...
import { createSpecialFeeTypeAction, updateSpecialFeeTypeAction, deleteSpecialFeeTypeAction, getSpecialFeeTypesPageDataAction, assignSpecialFeeTypeToStudentsAction, updateStudentFeeAction, deleteStudentFeeAssignmentAction } from './actions';
...
export default function ManageSpecialFeeTypesPage() {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    if (result.ok) {
      toast({ title: editingFeeType ? "Special Fee Updated" : "Special Fee Created", description: result.message });
      resetForm();
  ...
  return (
    <div className="flex flex-col gap-6">
      <PageHeader
        ...
        description="Create and assign one-off or extra charges (e.g., Late Fee, Lab Fee, Event Fee)."
        actions={
      ...
      <Tabs defaultValue="plans">
        <TabsList className="grid w-full grid-cols-3">
            <TabsTrigger value="plans">Special Fee Type Definitions</TabsTrigger>
        ...
        <TabsContent value="plans">
            <Card>
                <CardHeader>
                  <CardTitle className="flex items-center"><FileBadge className="mr-2 h-5 w-5" />Created Special Fee Types</CardTitle>
                </CardHeader>
                <CardContent>
                ...
                <CardHeader>
                 ...
                 <form onSubmit={handleAssignSubmit}>
                    <CardContent className="space-y-4">
                        ...
                        <div className="grid md:grid-cols-2 gap-4">
                           <div><Label>Special Fee Type to Assign</Label><Select value={assignFeeTypeId} onValueChange={setAssignFeeTypeId}><SelectTrigger><SelectValue placeholder="Select a fee type"/></SelectTrigger><SelectContent>{feeTypes.map(ft => <SelectItem key={ft.id} value={ft.id}>{ft.display_name}</SelectItem>)}</SelectContent></Select></div>
        ...
        <TabsContent value="assigned-log">
            <Card>
                 <CardHeader><CardTitle className="flex items-center">Assigned Special Fees Log</CardTitle></CardHeader>
                <CardContent>
