...
import { createSpecialFeeTypeAction, updateSpecialFeeTypeAction, deleteSpecialFeeTypeAction, getSpecialFeeTypesPageDataAction, assignSpecialFeeTypeToStudentsAction } from './actions';
...
export default function ManageSpecialFeeTypesPage() {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    const feeTypeData = { 
      ...
      amount: 0 // Default amount, as it will be set upon assignment
    };
  ...
  const handleDeleteFeeType = async (feeTypeId: string) => {
    ...
    toast({ title: result.ok ? "Fee Type Deleted" : "Error", description: result.message, variant: result.ok ? "destructive" : "destructive" });
  ...
  return (
    <div className="flex flex-col gap-6">
      <PageHeader
        ...
        description="Handle one-off or unique fees like 'Lost ID Card Fee' and assign them to students."
        actions={
      ...
      <Tabs defaultValue="assign">
        <TabsList className="grid w-full grid-cols-3">
            <TabsTrigger value="assign">Assign Special Fee</TabsTrigger>
            <TabsTrigger value="definitions">Special Fee Definitions</TabsTrigger>
        ...
        <TabsContent value="definitions">
            <Card>
                <CardHeader>
                  <CardTitle className="flex items-center"><IndianRupee className="mr-2 h-5 w-5" />Defined Special Fee Types</CardTitle>
                </CardHeader>
                <CardContent>
        ...
        <TabsContent value="assign">
            <Card>
                <CardHeader>
                    ...
                    <CardDescription>Assign a special fee to a whole class or individual students.</CardDescription>
                </CardHeader>
                 <form onSubmit={handleAssignSubmit}>
                    <CardContent className="space-y-4">
                        ...
                        <div className="grid md:grid-cols-2 gap-4">
                           <div><Label>Special Fee Type</Label><Select value={assignFeeTypeId} onValueChange={setAssignFeeTypeId}><SelectTrigger><SelectValue placeholder="Select a fee type"/></SelectTrigger><SelectContent>{feeTypes.map(ft => <SelectItem key={ft.id} value={ft.id}>{ft.name}</SelectItem>)}</SelectContent></Select></div>
        ...
        <TabsContent value="log">
            <Card>
                 <CardHeader><CardTitle className="flex items-center">Assignment Log</CardTitle></CardHeader>
                <CardContent>
