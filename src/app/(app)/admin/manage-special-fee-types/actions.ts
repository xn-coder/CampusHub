...
import type { FeeType, StudentFeePayment, PaymentStatus, Student, FeeCategory, ClassData } from '@/types';
...
export async function createSpecialFeeTypeAction(input: Omit<FeeType, 'id'>): Promise<{ ok: boolean; message: string; feeType?: FeeType }> {
  const supabase = createSupabaseServerClient();
  try {
...
export async function assignSpecialFeeTypeToStudentsAction(input: { student_ids: string[], fee_type_id: string, amount: number, due_date?: string, school_id: string }): Promise<{ ok: boolean; message: string; assignmentsCreated?: number }> {
    ...
    try {
        ...
        return { ok: true, message: `Fee assigned to ${count} student(s).`, assignmentsCreated: count || 0 };
    } catch(e: any) {
