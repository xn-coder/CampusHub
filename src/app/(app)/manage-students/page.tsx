...
import { Edit2, Search, Users, Activity, Save, Loader2, FileDown, UserX, AlertTriangle, UserCheck, FileText, ChevronLeft, ChevronRight, MoreHorizontal, ArrowDownUp } from 'lucide-react';
import { useToast } from "@/hooks/use-toast";
import { supabase } from '@/lib/supabaseClient'; 
import { terminateStudentAction, reactivateStudentAction, updateStudentAction, checkFeeStatusAndGenerateTCAction, getStudentActivityData } from './actions';
...
export default function ManageStudentsPage() {
  ...
  const [studentActivityData, setStudentActivityData] = useState<StudentWithActivity[]>([]);
  ...
  useEffect(() => {
   ...
   useEffect(() => {
    if (currentSchoolId) {
      ...
      if(activeTab === 'student-activity') {
        getStudentActivityData(currentSchoolId).then(res => {
            if(res.ok && res.data) setStudentActivityData(res.data as StudentWithActivity[]);
            else toast({title: "Error fetching activity", description: res.message, variant: "destructive"})
        });
      }
    }
  }, [currentSchoolId, showTerminated, fetchStudents, activeTab, toast]);
  ...
  const filteredActivityData = useMemo(() => studentActivityData.filter(student =>
    student.name.toLowerCase().includes(searchTerm.toLowerCase()) ||
    (student.email && student.email.toLowerCase().includes(searchTerm.toLowerCase()))
  ), [studentActivityData, searchTerm]);


  const paginatedStudents = useMemo(() => {
    const dataToPaginate = activeTab === 'list-students' ? filteredStudents : filteredActivityData;
    const startIndex = (currentPage - 1) * ITEMS_PER_PAGE;
    return dataToPaginate.slice(startIndex, startIndex + ITEMS_PER_PAGE);
  }, [filteredStudents, filteredActivityData, currentPage, activeTab]);
  const totalPages = Math.ceil((activeTab === 'list-students' ? filteredStudents.length : filteredActivityData.length) / ITEMS_PER_PAGE);
  ...
  const handleDownloadCsv = () => {
    const dataToDownload = activeTab === 'list-students' ? filteredStudents : filteredActivityData;
    if (dataToDownload.length === 0) {
        toast({ title: "No Data", description: "There are no students to download for the current filter.", variant: "destructive"});
        return;
    }
    const isActivityTab = activeTab === 'student-activity';

    const headers = isActivityTab 
        ? ["Name", "Roll Number", "Email", "Class", "Status", "Last Login", "Assignments Submitted", "Attendance %"]
        : ["Name", "Roll Number", "Email", "Class", "Status", "Student UUID"];

    const csvRows = [
        headers.join(','),
        ...dataToDownload.map(student => {
            ...
            if(isActivityTab) {
                const activityStudent = student as StudentWithActivity;
                row.push(
                    `"${formatDateSafe(activityStudent.lastLogin)}"`,
                    String(activityStudent.assignmentsSubmitted || 0),
                    String(activityStudent.attendancePercentage || 0)
                );
            } else {
                row.push(`"${student.id}"`);
            }
    ...
    link.setAttribute("download", `student_roster_${activeTab}_${new Date().toISOString().split('T')[0]}.csv`);
        ...
        <TabsContent value="student-activity">
            <Card>
                <CardHeader>
                    ...
                    <CardDescription>Review key engagement metrics for students in your school.</CardDescription>
                </CardHeader>
                <CardContent>
                    <div className="mb-4 flex flex-wrap items-center gap-4">
                       <div className="flex-grow flex items-center gap-2">
                            ...
                            <TableBody>
                                {paginatedStudents.map(student => (
                                    <TableRow key={student.id}>
                                        <TableCell className="font-medium">{student.name}</TableCell>
                                        <TableCell>{student.email}</TableCell>
                                        <TableCell>{formatDateSafe((student as StudentWithActivity).lastLogin)}</TableCell>
                                        <TableCell className="text-right">{(student as StudentWithActivity).assignmentsSubmitted}</TableCell>
                                        <TableCell className="text-right">{(student as StudentWithActivity).attendancePercentage}%</TableCell>
                                    </TableRow>
                                ))}
                            </TableBody>
                        </Table>
                     ) : <p className="text-center text-muted-foreground py-4">No student activity data available.</p>}
                </CardContent>
                 {totalPages > 1 && (
                    <CardFooter className="flex justify-end items-center gap-2">
                        <Button variant="outline" size="sm" onClick={() => setCurrentPage(p => p - 1)} disabled={currentPage === 1}>
