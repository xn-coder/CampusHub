...
async function sendEmail(options: EmailOptions): Promise<{ success: boolean; message: string }> {
  if (!isEmailJsConfigured || !SERVICE_ID || !TEMPLATE_ID || !USER_ID) {
    ...
    console.log("--- END MOCK EMAIL (from /src/app/(app)/teacher/grade-assignments/actions.ts) ---");
  ...
  for (const recipientEmail of sendToAddresses) {
    ...
    try {
      ...
      console.error(`Failed to send email to ${recipientEmail} via EmailJS from teacher/grade-assignments/actions.ts. Status: ${error?.status}, Text: ${error?.text}. Full error:`, error);
